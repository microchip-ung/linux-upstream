...
static int spinand_cmd_24bit(struct spi_device *spi_nand, u8 cmd_code, u32 addr)
{
        struct spinand_cmd cmd = {0};
...
{
        return spinand_cmd_24bit(spi_nand, CMD_READ, page_id);
}
...
{
        return spinand_cmd_24bit(spi_nand, CMD_PROG_PAGE_EXC, page_id);
}
...
{
        return spinand_cmd_24bit(spi_nand, CMD_ERASE_BLK, block_id);
}
...
static int spinand_write_subpage_hwecc(struct mtd_info *mtd,
                                       ...
                                       uint32_t data_len, const u8 *buf,
...
MODULE_DESCRIPTION("SPI NAND driver for Macronix");
