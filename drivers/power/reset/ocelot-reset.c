				    ...
				    const struct reset_mscc_props *props)
{
	...
	set_bits(icpu_base + props->protect_reg_off, props->vcore_protect);
				 ...
				 unsigned long mode, void *cmd)
{
	...
	clr_bits(ctx->icpu_base + ctx->props->protect_reg_off,
...
static int ocelot_reset_probe(struct platform_device *pdev)
{
	...
	ctx->cpu_reset_only = of_property_read_bool(np, "mscc,cpu-reset-only");
