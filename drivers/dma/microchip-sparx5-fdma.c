			       ...
			       struct microchip_sparx5_fdma *priv)
{
	...
	vaccess_chip_id = microchip_sparx5_vcore_read_reg(priv,
	...
	res = microchip_sparx5_buffer_access_test(priv, message, 160, 0, buffer, phys);
	...
	res = microchip_sparx5_buffer_access_test(priv, message, 160, 1, buffer, phys);
	kfree(buffer);
	if (res) {
		goto error;
	}
