...
#define UFDMA_DCACHE_LINE_SIZE_BYTES 32 /* Data Cache Line size. Currently, it's the sam on all supported platforms */
...
/**
 * Chip architecture. Only needed for Jaguar2AB, Jaguar2C, and Serval-T,
 * which use the same code base, but different register offsets.
 */
typedef enum {
...
typedef struct {
    ...
    int (*poll)(struct ufdma_state_s *state);

    // CIL function for checking whether frames are received
    int (*rx_poll)(struct ufdma_state_s *state);

    // CIL function for checking whether frames are transmitted
    int (*tx_poll)(struct ufdma_state_s *state);
...
typedef struct {
     ...
     */
    int (*rx_frm)(struct ufdma_state_s *state, u32 chip_no);
