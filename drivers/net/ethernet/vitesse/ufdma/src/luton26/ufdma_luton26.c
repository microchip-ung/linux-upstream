 ...
 * CIL_poll_do()
 */
static int CIL_poll_do(ufdma_state_t *state, BOOL rx, BOOL tx)
{
    ...
    u32 intr_block_done = REG_RD(VTSS_FDMA_INTR_STATUS_BLOCK) & ch_mask; // The extraction channels use block done interrupts
    u32 intr_tfr_done   = REG_RD(VTSS_FDMA_INTR_STATUS_TFR)   & ch_mask; // Both injection and extraction channels use transfer done interrupts.
 ...
 * CIL_poll()
 */
static int CIL_poll(ufdma_state_t *state)
{
    return CIL_poll_do(state, TRUE, TRUE);
}

/**
 * CIL_tx_poll()
 */
static int CIL_tx_poll(ufdma_state_t *state)
{
    return CIL_poll_do(state, FALSE, TRUE);
}

/**
 * CIL_rx_poll()
 */
static int CIL_rx_poll(ufdma_state_t *state)
{
    return CIL_poll_do(state, TRUE, FALSE);
}
...
static int CIL_init(vtss_ufdma_platform_driver_t *self, vtss_ufdma_init_conf_t *init_conf)
{
    ...
    state->cil.tx_poll               = CIL_tx_poll;
    state->cil.rx_poll               = CIL_rx_poll;
