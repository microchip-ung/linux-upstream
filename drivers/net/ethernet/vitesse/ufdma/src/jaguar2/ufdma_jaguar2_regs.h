...
// ServalT register offsets are 8 lower than Jaguar2 register offsets.
...
#define VTSS_QFWD_SYSTEM_FRAME_COPY_CFG(ri)                VTSS_IOREG(VTSS_TO_QFWD, (state->cil.chip_arch == CHIP_ARCH_SERVAL_T ? 0x1120 : state->cil.chip_arch == CHIP_ARCH_JAGUAR_2C ? 0x003a : 0x111f) + (ri))
