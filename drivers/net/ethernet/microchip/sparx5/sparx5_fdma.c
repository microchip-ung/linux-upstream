...
static void sparx5_fdma_rx_activate(struct sparx5 *sparx5, struct sparx5_rx *rx)
{
	...
	spx5_rmw(BIT(rx->channel_id), FDMA_CH_ACTIVATE_CH_ACTIVATE,
...
static void sparx5_fdma_tx_activate(struct sparx5 *sparx5, struct sparx5_tx *tx)
{
	...
	spx5_rmw(BIT(tx->channel_id), FDMA_CH_ACTIVATE_CH_ACTIVATE,
		 sparx5, FDMA_CH_ACTIVATE);
}
...
static void sparx5_fdma_rx_reload(struct sparx5 *sparx5, struct sparx5_rx *rx)
{
	...
	spx5_rmw(BIT(rx->channel_id), FDMA_CH_RELOAD_CH_RELOAD,
		 sparx5, FDMA_CH_RELOAD);
}

static void sparx5_fdma_tx_reload(struct sparx5 *sparx5, struct sparx5_tx *tx)
{
	/* Reload the TX channel */
	spx5_rmw(BIT(tx->channel_id), FDMA_CH_RELOAD_CH_RELOAD,
		 sparx5, FDMA_CH_RELOAD);
}
...
static bool sparx5_fdma_rx_get_frame(struct sparx5 *sparx5, struct sparx5_rx *rx)
{
	...
	pr_debug("%s:%d: db_hw: %#lx\n", __func__, __LINE__, (unsigned long)db_hw);
	...
	pr_debug("%s:%d: port: %02d\n", __func__, __LINE__, fi.src_port);
	if (!port || !port->ndev) {
...
static int sparx5_fdma_rx_alloc(struct sparx5 *sparx5)
{
	...
	pr_debug("DCB: %s %#lx, %#lx\n", __func__, (unsigned long)rx->dcb_entries,
		 (unsigned long)rx->dma);
	...
	for (idx = 0; idx < FDMA_DCB_MAX; ++idx) {
		...
		pr_debug("DCB: %s virt: %#lx\n", __func__, (unsigned long)dcb);
...
static int sparx5_fdma_tx_alloc(struct sparx5 *sparx5)
{
	...
	pr_debug("DCB: %s %#lx, %#lx\n", __func__, (unsigned long)tx->curr_entry,
		 (unsigned long)tx->dma);
	/* Now for each dcb allocate the db */
	for (idx = 0; idx < FDMA_DCB_MAX; ++idx) {
...
int sparx5_fdma_start(struct sparx5 *sparx5)
{
	...
	pr_info("%s:%d: irq: %d\n", __func__, __LINE__, sparx5->fdma_irq);
	/* Reset FDMA state */
